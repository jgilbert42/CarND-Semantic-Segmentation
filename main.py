...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    vgg_layer7_out = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, strides=(1,1), padding='same',
    ...
    net = tf.layers.conv2d_transpose(vgg_layer7_out, num_classes, 4, strides=(2,2), padding='same',
            kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    net = tf.add(net, vgg_layer4_out)
    net = tf.layers.conv2d_transpose(net, num_classes, 4, strides=(2,2), padding='same',
            kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    net = tf.add(net, vgg_layer3_out)
    net = tf.layers.conv2d_transpose(net, num_classes, 16, strides=(8,8), padding='same',
            kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            ...
            tf.logging.info('Epoch: {}, Batch: {}, Loss: {}'.format(epoch, batch, loss_value))
